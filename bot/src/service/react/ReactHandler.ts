...
export class ReactHandler {
    ...
    constructor(
        ...
        const handlers = [pinReactHandler];
    ...
    public handleAdd(messageReaction: MessageReaction): void | Promise<void> {
        this.logger.debug(
            `ReactHandler: Got added reaction ${messageReaction.emoji.name} on ${messageReaction.message.id}`,
        );
        this.guildSettingsRepo.saveDefaultGuild(
    ...
    public handleRemove(
        ...
        this.logger.debug(
            `ReactHandler: Got removed reaction ${messageReaction.emoji.name} on ${messageReaction.message.id}`,
        );
        this.guildSettingsRepo.saveDefaultGuild(
            messageReaction.message.guild?.id,
        );
